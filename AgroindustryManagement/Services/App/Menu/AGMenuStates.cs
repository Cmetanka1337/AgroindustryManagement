namespace AgroindustryManagement.Services.App.Menu;

public static class AGMenuState
{
    public const string MainMenuState = "Main";
    public const string FieldMenuState = "Field";
    public const string WorkerMenuState = "Worker";
    public const string MachineMenuState = "Machine";
    public const string InventoryItemMenuState = "InventoryItem";
    public const string WorkerTaskMenuState = "WorkerTask";
}