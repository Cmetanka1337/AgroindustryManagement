using AgroindustryManagement.Models;
    ...
    public class AGViewService
    {
        ...
        public Field GetFieldDataFromUser()
        {
            Console.WriteLine("Enter the details for the new field:");
        
            // Get Area
            double area;
            while (true)
            {
                Console.Write("Enter field area (in hectares): ");
            ...
            // Get Culture Type
            Console.WriteLine("Available Culture Types:");
            foreach (var culture in Enum.GetValues(typeof(CultureType)))
            {
            ...
            while (true)
            {
                Console.Write("Enter culture type: ");
                if (Enum.TryParse(Console.ReadLine(), true, out cultureType) && Enum.IsDefined(typeof(CultureType), cultureType))
                {
                    break;
                }
                Console.WriteLine("Invalid input. Please enter a valid culture type.");
            }
        
            // Get Field Status
            Console.WriteLine("Available Field Statuses:");
            foreach (var statusValue in Enum.GetValues(typeof(FieldStatus)))
            {
            ...
            while (true)
            {
                Console.Write("Enter field status: ");
                if (Enum.TryParse(Console.ReadLine(), true, out status) && Enum.IsDefined(typeof(FieldStatus), status))
                {
                    break;
                }
                Console.WriteLine("Invalid input. Please enter a valid field status.");
            }
        
            // Get Workers
            var workers = new List<Worker>();
            Console.WriteLine("Enter worker IDs assigned to this field (comma-separated, or leave empty if none):");
            var workerInput = Console.ReadLine();
            if (!string.IsNullOrWhiteSpace(workerInput))
            {
                // var workerIds = workerInput.Split(',');
                // foreach (var workerId in workerIds)
                // {
                //     if (int.TryParse(workerId.Trim(), out var id))
                //     {
                //         var worker = App.DatabaseService.GetWorkerById(id); // Assuming a method to fetch workers by ID
                //         if (worker != null)
                //         {
                //             workers.Add(worker);
                //         }
            ...
            Console.WriteLine("Enter machine IDs assigned to this field (comma-separated, or leave empty if none):");
            var machineInput = Console.ReadLine();
            if (!string.IsNullOrWhiteSpace(machineInput))
            {
                // var machineIds = machineInput.Split(',');
                // foreach (var machineId in machineIds)
                // {
                //     if (int.TryParse(machineId.Trim(), out var id))
                //     {
                //         var machine = App.DatabaseService.GetMachineById(id); // Assuming a method to fetch machines by ID
                //         if (machine != null)
                //         {
            ...
            Console.WriteLine("Enter task IDs assigned to this field (comma-separated, or leave empty if none):");
            var taskInput = Console.ReadLine();
            if (!string.IsNullOrWhiteSpace(taskInput))
            {
                // var taskIds = taskInput.Split(',');
                // foreach (var taskId in taskIds)
                // {
                //     if (int.TryParse(taskId.Trim(), out var id))
                //     {
                //         var task = App.DatabaseService.GetWorkerTaskById(id); // Assuming a method to fetch tasks by ID
                //         if (task != null)
                //         {
                //             tasks.Add(task);
                //         }
